...
def train_and_evaluate(config):
    ...
    #                    shuffle_files=True,
    #                    data_dir=config.data_dir)
    builder = tfds.ImageFolder('/home/gdevesan_gmail_com/datasets/processed_images')
    # print(builder.info)  # num examples, labels... are automatically calculated
    ds_train = builder.as_dataset(split='train', shuffle_files=True)
    ds_val = builder.as_dataset(split='test', shuffle_files=True)
    ...
    for epoch in range(epoch_offset, config.num_epochs):
        ...
        accuracy = 0.0
        n = 0
        for batch in tfds.as_numpy(ds_val):
            image = batch['image'].astype(dtype)
            label = batch['label'].astype(dtype)
            if image.shape[0] % num_devices != 0:
                continue
