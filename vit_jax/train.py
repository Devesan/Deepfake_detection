...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str):
  ...
  for step, batch in zip(
    ...
    if ((config.eval_every and step % config.eval_every == 0) or
      ...
      wandb.log({'test/accuracy': accuracy_test}, step=step)
      wandb.log({'train/learning_rate': lr}, step=step)
      wandb.log({'test/img_sec_core': img_sec_core_test}, step=step)
      wandb.log({'train/img_sec_core': img_sec_core_train}, step=step)
      wandb.log({'train/loss': float(flax.jax_utils.unreplicate(loss_repl))}, step=step)
