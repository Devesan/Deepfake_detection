...
#     """
#     fpr, tpr, thresholds = sklearn.metrics.roc_curve(actual, prediction, pos_label=1)
  ...
  for step, batch in zip(
    ...
    if ((config.eval_every and step % config.eval_every == 0) or
      ...
      for test_batch in input_pipeline.prefetch(ds_test, config.prefetch):
        ...
        label = np.argmax(test_batch['label'],axis=-1).mean()
        print('pred and label',pred,label)
        accuracy = np.argmax(logits,
                       axis=-1) == np.argmax(test_batch['label'],
                                             axis=-1).mean()
        f1_score_1  = calc_f1(pred,label)
        auc_score_1 = calc_auc(pred,label)
        f1s.append(f1_score_1)
        aucs.append(auc_score_1)
        accuracies.append(accuracy)
      accuracy_test = np.mean(accuracies)
      auc_test = np.mean(aucs)
      f1_test = np.mean(f1s)
      img_sec_core_test = (
